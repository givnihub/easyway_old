...
class Exam extends Controller
{
   ...
   if($tradegroup!='' && $trade!='' && $subject!='' && $chapter!='' && $topic!=''){
    ...
    public function addquestion(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $data = array(
                ...
                'opt_a' =>  trim($req->input('option_a')),
                'opt_b' => trim($req->input('option_b')),
                'opt_c' => trim($req->input('option_c')),
                'opt_d' => trim($req->input('option_d')),
                'opt_e' =>trim ($req->input('option_e')),
                'explanation' => trim($req->input('explanation')),
    ...
    {
        $deleteOldQuestion=DB::table("onlineexam_questions")->where('examid',$req->input('examid'))->delete();
       if(count($req->input('checkboxid'))>0){
 for ($i = 0; $i < count($req->input('checkboxid')); $i++) {
         ...
         echo '<div class="alert alert-success">
            <strong>Success!</strong> Question updated succesfully.
          </div>';
    ...
    {
        $data['examTitle']=DB::table("onlineexam")->where("id",$id)->first(['exam']);
