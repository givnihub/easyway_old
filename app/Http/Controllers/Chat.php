...
//group discussion message
    public function DiscussionGroup(Request $req, $id, $courseId)
    {
        $data['userId'] = $id;
         $data['courseId'] = $courseId;
        $data['users']=DB::table('students')->where('id',$id)->first(['firstname']);
        return view('chat/DiscussionGroup', $data);
    }

    public function newGDMessage(Request $req)
    {
        ...
        $insert = DB::table("GroupDiscussionMessage")->insert($data);
    }


     public function chatGDMessage(Request $req)
    {
          $userId = $req->input('userId');
         $courseId = $req->input('courseId');


        $row = DB::table("GroupDiscussionMessage as c")
            ->leftJoin("students as u", "u.id", "=", "c.chat_user_id")
            ->where("c.courseId", $courseId)
            ->whereRaw("NOT FIND_IN_SET($userId, is_read)")
            ->limit(1)->orderBy('c.id', 'desc')->get(['c.id', 'c.chat_user_id', 'c.courseId', 'c.message', 'c.is_first', 'c.is_read', 'c.created_at', 'u.firstname']);
        ...
        foreach ($row as $run) {
            ...
            $update = DB::table('GroupDiscussionMessage')->where('id', $run->id)->update($datas);
            $chat_type = ($run->chat_user_id != $userId) ? 'sent' : 'replies';
...
<? }
    ...
    public function allGDMessage(Request $req)
    {
        $userId = $req->input('userId');
        $courseId = $req->input('courseId');
        $row = DB::table("GroupDiscussionMessage as c")
            ->leftJoin("students as u", "u.id", "=", "c.chat_user_id")
            ->where("c.courseId", $courseId)->orderBy('c.id', 'asc')->get(['c.id', 'c.chat_user_id', 'c.courseId', 'c.message', 'c.is_first', 'c.is_read', 'c.created_at', 'u.firstname']);


        foreach ($row as $run) {
            $uid = $run->id;
            $checkIsRead = $query = DB::table('GroupDiscussionMessage')
                ->where('id', $uid)
                ->whereRaw("FIND_IN_SET($userId, is_read)")
                ->count();
            if ($checkIsRead == 0) {
                ...
                $update = DB::table('GroupDiscussionMessage')->where('id', $uid)->update($datas);
            }
