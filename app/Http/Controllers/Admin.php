...
class Admin extends Controller
{
  ...
  if ($req->file('image') != '') {
         ...
         $imageUrl = $req->file('image')->move('public/uploads/gallery/media', $image);
         }
         $data = array(
            'title' => trim($req->input('title')),
...
if($req->input('delid')!=''){
   ...
   public function liveVideos(Request $req){
      $cat_id= DB::table("submenu")->where("link",'LIKE', "%".$req->segment('2')."%")->first(['id'])  ;
      $data['perm_cat_id']= $cat_id->id;
      $data['role_id']=session()->get('userInfo')['role'];
      $data['list']=DB::table('videos')->where('is_live','yes')->orderBy('live_date','asc')->get(['id','title','video_id','live_date','description','course_id']);
      return view('admin.liveVideos',$data);
  
   }
      public function coupons(Request $req)
   {
      if ($req->input('delid')) {
         ...
         $update = DB::table("courses")->where('id', $delid)->update($datas);
      }
      if ($_SERVER['REQUEST_METHOD'] == 'POST') {
         $courseId = $req->input('course');
         ...
         $data = array(
            ...
            'validTill' => $req->input('validTill'),
            'limitOfUse' => $req->input('limitOfUse'),
            'status' => $req->input('status'),
         );
         ...
         if ($courseId[0] == 'all') {
            ...
            $update = DB::table("courses")->update($datas);
         } else {
            for ($i = 0; $i < count($courseId); $i++) {
               $update = DB::table("courses")->where('id', $courseId[$i])->update($datas);
            }
         }
      }

      $data['courses'] = DB::table("courses")->get(['id', 'title', 'couponCode', 'status']);
