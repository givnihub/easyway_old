...
class Staff extends Controller
{
   ...
   public function staff(Request $req)
   {
      ...
      if ($_SERVER['REQUEST_METHOD'] == 'POST') {
         ...
         $role = $req->input('role');
         if ($role != '') {
            $data['list'] = DB::select('select id,name,employee_id,role,department,designation,qualification,image,email,contact_no,emergency_contact_no,commision,discount from staff where role=' . $role . ' and status=1 order by id asc');
         }
         if ($search_text != '') {
            $data['list'] = DB::select('select id,name,employee_id,role,department,designation,qualification,image,email,contact_no,emergency_contact_no,commision,discount from staff where name like "%' . $search_text . '%" OR surname like "%' . $search_text . '%" and status=1 order by id asc');
         }
      } else {
         $data['list'] = DB::select('select id,name,employee_id,role,department,designation,qualification,image,email,contact_no,emergency_contact_no,commision,discount from staff where status=1 order by id asc');
      }
      $cat_id= DB::table("submenu")->where("link",'LIKE', "%".$req->segment('2')."%")->first(['id'])  ;
      $data['perm_cat_id']= $cat_id->id;
      $data['role_id']=session()->get('userInfo')['role'];
   ...
   public function profile(Request $req)
   {
      if($_SERVER['REQUEST_METHOD']=='POST'){
         ...
         $data=array(
            'disable_at'=>$req->input('date'),
            'reason'=>$req->input('reason'),
            'status'=>$req->input('status'),
         );
         $disable=DB::table("staff")->where("id",$req->input('staff_id'))->update($data);
         $req->session()->flash('success', 'Staff status changed now...');
         return redirect($_SERVER['HTTP_REFERER']); 
      }
   ...
   public function attendance(Request $req)
   {
      ...
      if ($req->input('user_id') != '') {
         ...
         $data['list'] = DB::select('select id,name,surname,employee_id,role from staff where status=1 and role=' . $req->input('user_id'));
      }
      $cat_id= DB::table("submenu")->where("link",'LIKE', "%".$req->segment('2')."%")->first(['id'])  ;
      $data['perm_cat_id']= $cat_id->id;
      $data['role_id']=session()->get('userInfo')['role'];
   ...
   public function disablestafflist(Request $req)
   {
      ...
      if ($_SERVER['REQUEST_METHOD'] == 'POST') {
         ...
         $role = $req->input('role');
         if ($role != '') {
            $data['list'] = DB::select('select id,name,employee_id,role,department,designation,qualification,image,email,contact_no,emergency_contact_no,commision,discount from staff where role=' . $role . ' and status=0 order by id asc');
         }
         if ($search_text != '') {
            $data['list'] = DB::select('select id,name,employee_id,role,department,designation,qualification,image,email,contact_no,emergency_contact_no,commision,discount from staff where name like "%' . $search_text . '%" OR surname like "%' . $search_text . '%" and status=0 order by id asc');
         }
      } else {
         $data['list'] = DB::select('select id,name,employee_id,role,department,designation,qualification,image,email,contact_no,emergency_contact_no,commision,discount from staff where status=0 order by id asc');
      }
      $cat_id= DB::table("submenu")->where("link",'LIKE', "%".$req->segment('2')."%")->first(['id'])  ;
      $data['perm_cat_id']= $cat_id->id;
      $data['role_id']=session()->get('userInfo')['role'];
